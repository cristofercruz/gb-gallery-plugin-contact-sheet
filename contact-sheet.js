...
(function contactSheetPlugin() {
  ...
  const FRAME_MODE_LABELS = {
    keep: 'Keep frame',
    crop: 'Crop frame',
    square_black: 'Make image squared (black)',
  ...
  class ContactSheetPlugin {
    constructor(env, config) {
      ...
      this.configParams = {
        ...
        frameMode: {
          label: 'Frame handling: "keep", "crop", "square_black" or "square_white" (blank = use the app\u2019s global frame setting)',
          type: 'string',
        },
        scaleGapMargin: {
    ...
    // Render every frame mode (at the given scale) so the dialog preview can
    // switch frame modes instantly. Returns { mode: cells[] }. Each image's
    // metadata is fetched once and shared across all modes; the chosen mode's
    // set is reused for the export, so the frames are never rendered twice.
    ...
    withSelection(images) {
      ...
      return this.loadFrameVariants(images, loadedScaleFactor)
        .then((cellsByMode) => {
          ...
          // whose label spells out the app's current global frame behaviour. The
          // concrete modes follow FRAME_MODES order.
          const frameOptions = [
            { name: `Use app default setting (${FRAME_MODE_LABELS[appFrame]})`, value: 'default' },
            ...FRAME_MODES.map((mode) => ({ name: FRAME_MODE_LABELS[mode], value: mode })),
